...
<table class="table table-bordered">
	<tr>
		<form method="post" action="index.php?page=search_center">
		<td colspan="8">
		<input type="text"  placeholder="Search Center" name="searchGroup" class="form-control" required />
		</td>
		<td colspan="8">
		<input type="submit" value="Search Center" name="sub" class="btn btn-success" />
		</td>
		</form>
	</tr>
...
<?php
  ...
  $result = $conn->query($sql);
   ...
   while($row = $result->fetch_assoc()) {
    echo "<tr><td>U" . $row["c_id"]. "</td><td>" . $row["c_location"] . "</td>";
